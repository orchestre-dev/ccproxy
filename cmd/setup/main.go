...
func generateEnvFile(provider *ProviderSetup, config map[string]string) error {
	...
	// Write header
	fmt.Fprintf(file, "# CCProxy Configuration - %s Provider\n", provider.DisplayName)
	fmt.Fprintf(file, "# Generated by CCProxy Setup Assistant\n")
	fmt.Fprintf(file, "# %s\n\n", provider.Description)

	// Write main provider setting
	fmt.Fprintf(file, "# Provider Selection\n")
	...
	fmt.Fprintf(file, "# Uncomment these lines to use CCProxy with Claude Code\n")
	fmt.Fprintf(file, "# export ANTHROPIC_BASE_URL=http://localhost:7187\n")
	fmt.Fprintf(file, "# export ANTHROPIC_API_KEY=NOT_NEEDED\n")
