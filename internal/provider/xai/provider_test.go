...
func TestNewProvider(t *testing.T) {
	...
	if provider.GetName() != "xai" {
		t.Errorf("Expected provider name 'xai', got %s", provider.GetName())
	}

	if provider.GetModel() != "grok-beta" {
		t.Errorf("Expected model 'grok-beta', got %s", provider.GetModel())
	}
}
...
func TestProvider_CreateChatCompletion_Success(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{
			...
			"choices": [{
				...
				"message": {
					"role": "assistant",
					"content": "Hello! I'm Grok. How can I assist you today?"
				},
				"finish_reason": "stop"
			}],
			"usage": {
	...
	expectedContent := "Hello! I'm Grok. How can I assist you today?"
...
func TestProvider_InterfaceCompliance(t *testing.T) {
	...
	if provider.GetName() != "xai" {
		t.Errorf("Expected GetName() to return 'xai', got %s", provider.GetName())
	}

	if provider.GetModel() != "grok-beta" {
		t.Errorf("Expected GetModel() to return 'grok-beta', got %s", provider.GetModel())
	}
	...
	if provider.GetBaseURL() != "https://api.x.ai/v1" {
