...
func TestProvider_InterfaceCompliance(t *testing.T) {
	...
	if provider.GetName() != "openai" {
		t.Errorf("Expected GetName() to return 'openai', got %s", provider.GetName())
	}

	if provider.GetModel() != "gpt-4" {
		t.Errorf("Expected GetModel() to return 'gpt-4', got %s", provider.GetModel())
	}
	...
	if provider.GetBaseURL() != "https://api.openai.com/v1" {
